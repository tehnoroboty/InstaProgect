import { Message, SendMessageArgs } from '@/src/entities/messenger/types'
...
class SocketIoApi {
  ...
  createConnection(dispatch: AppDispatch) {
    ...
    this.socket.on(WS_EVENT_PATH.MESSAGE_SEND, (data: Message) => {
      ...
      dispatch(
        messengerApi.util.updateQueryData(
          'getMessagesByUser',
          { dialoguePartnerId: newMessage.receiverId },
          draft => {
            const existsIndex = draft.items.findIndex(item => item.id === data.id)
  ...
  sendMessage(message: SendMessageArgs) {
    if (this.socket) {
      this.socket.emit(WS_EVENT_PATH.RECEIVE_MESSAGE, message)
    }
  }
}
